...
# --- 3. PROFESSIONAL HTML EMAIL ENGINE ---
def send_daily_summary_email(items_list):
    try:
        ...
        # Create a Multi-part message (allows HTML)
        ...
        <body>
            ...
            <p>The following items have reached the re-order level (Stock ≤ 2):</p>
            <table>
                <tr>
        ...
        # Add rows, filtering out 'nan' garbage data
        valid_items_count = 0
        for item in items_list:
            # Skip if name is 'nan' or empty
            if str(item['name']).lower() == 'nan' or not str(item['name']).strip():
                continue
        ...
        html_content += """
            ...
            <div class="footer">
                <p>⚠️ This is an automated alert generated by the EMD Material Dashboard.<br>
                Please arrange for procurement immediately.</p>
            </div>
        </body>
        </html>
        """
        
        # Only send if we have valid items (not just empty rows)
...
# Remove 'nan' rows from Critical List for cleaner display
...
with tab2:
    if not crit.empty:
        ...
        st.download_button(
            ...
            data=csv,
            file_name=f"Critical_Stock.csv",
            mime='text/csv',
        )
    else:
...
if is_time and items_exist and not email_already_sent_today:
    ...
    for _, r in crit.iterrows():
        # Double check to prevent 'nan' in email list
        if str(r['MATERIAL DISCRIPTION']).lower() != 'nan':
            clist.append({'name': r['MATERIAL DISCRIPTION'], 'qty': r['LIVE STOCK'], 'loc': r['LOCATION']})
    
    if clist: # Only send if list is not empty
